...
if (!('addEventListener' in window)) { window.addEventListener = window.attachEvent; }
...
var SFX = {};
var AUDIO_SPRITES = {
  step:  'data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABYB...==',
  door:  'data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABYB...==',
  pick:  'data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABYB...==',
  shot:  'data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABYB...=='
};
// NB: ho accorciato i base64 per brevità. Se li vuoi subito pronti, dimmelo e te li fornisco full.
// In alternativa, punta a ./assets/sfx/*.wav
...
function onKey(e, down){
  ...
  if (typeof code === 'number') code = 'Key'+code; // legacy
  Keys[code] = down;
...
if (touch){
  ...
  touch.addEventListener('touchend', function(e){
    var t = e.target;
    if (t && t.getAttribute('data-k')) Keys[t.getAttribute('data-k')] = false;
  });
}
...
var TILE = 16;           // unità base (piastrelle)
var GRAV = 0.7;
var FRICTION = 0.85;
var MAX_LIVES = 3;
...
/* Ogni livello definisce:
   ...
   - doors: array di porte {x,floor,red}
   - elevators: array {x, yMin, yMax, speed}
   - enemies: array {x,floor,dir}
   Le coordinate X sono in pixel, floor 0 = tetto, floor N-1 = piano terra
*/
var LEVELS = [
  {
    floors: 8,
...
Hero.prototype.update = function(world){
  var speed = 1.6;
  if (this.inElevator) {
    // dentro ascensore: su/giù, uscita con destra/sinistra
  ...
  // uscita in auto (fondo) se intel raccolti
...
Enemy.prototype.update = function(world){
  ...
  // fuoco sporadico verso eroe sulla stessa linea
  if (this.reload>0) this.reload--;
  var hero = world.hero;
  if (this.reload===0 && Math.abs(this.y-hero.y)<4){
    var dir = hero.x>this.x ? 1:-1;
    world.spawnBullet(this.x+this.w/2, this.y+6, dir, true);
...
Door.prototype.tryOpen = function(hero,world){
  ...
  if (near && Keys['KeyZ']){ // tasto azione
    this.opened = true;
...
function startLevel(idx){
  ...
  state.intelTotal = world.intelTotal;
  state.intelGot = 0;
  // HUD
...
function bootstrap(){
  ...
  // piccolo hint poi scompare
  setTimeout(function(){
    var b = document.getElementById('banner');
    if (b) b.style.display = 'none';
  }, 5000);

  // update HUD periodico
  setInterval(updateHud, 200);

  // su iOS8 serve un primo tap per sbloccare audio
  canvas.addEventListener('touchstart', function(){ audioUnlocked = true; }, {passive:true});
}
bootstrap();
